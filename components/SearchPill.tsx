...
export default function SearchPill({ onClick }: { onClick: () => void }) {
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate text-[13.5px] font-medium text-white/55">
        Search sets, artists, parking, prayer…
      </span>
      <span className="hidden shrink-0 rounded-md border border-white/15 px-1.5 py-0.5 text-[10px] font-bold text-white/55 sm:block">
        ⌘K
      </span>
    </button>
  );
